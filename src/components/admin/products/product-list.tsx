    ...
    return (
      <div className="flex items-center justify-center py-6 md:py-8">
        <div className="text-center">
          ...
          <p className="text-xs md:text-sm text-muted-foreground">{tCommon('loadingProducts')}</p>
        </div>
      </div>
    )
  }
  ...
  if (products.length === 0) {
    return (
      <div className="text-center py-6 md:py-8 px-4">
        ...
        <h3 className="text-base md:text-lg font-semibold mb-2">{tCommon('noProductsFound')}</h3>
        <p className="text-muted-foreground mb-4 text-xs md:text-sm max-w-md mx-auto">
          {hasFilters 
            ? t('noProductsFoundFiltered') 
            : t('noProductsFoundEmpty')
          }
        </p>
